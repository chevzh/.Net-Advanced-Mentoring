...
namespace StockExchange.Task3
{
    public abstract class Player
    {
            ...
            if (this != player && Id != player.Id)
            {
                ...
                if (player.BuyOffers.TryGetValue(stockName, out offerNumber))
                {
                    if (offerNumber == numberOfShares)
                    {
                        ...
                        Update("Sell", numberOfShares, player);
            ...
            if (this != player && Id != player.Id)
            {
                ...
                if (player.SellOffers.TryGetValue(stockName, out offerNumber))
                {
                    if (offerNumber == numberOfShares)
                    {
                        ...
                        Update("Buy", numberOfShares, player);
        ...
        private void Update(string dealType, int numberOfShares, Player player)
        {
            this.Update(dealType, numberOfShares);
            player.Update(dealType == "Sell" ? "Buy" : "Sell", numberOfShares);
        }
    }
}
